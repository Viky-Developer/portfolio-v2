...
>
	...
	<div
		class="flex min-w-0 items-center justify-between gap-3 border-b border-divider bg-surface-primary px-3 py-2.5 select-none sm:px-4"
	>
		<div class="flex min-w-0 items-center gap-2">
			...
			{#if title}
				<span
					class="ml-1 truncate font-mono text-[11px] font-medium text-text-muted sm:ml-2 sm:text-[12px]"
				>
					{title}
				</span>
			{/if}
	...
	<div class="code-scroll w-full min-w-0 flex-1 overflow-x-auto p-3 sm:p-4">
		{#if children}
			{@render children()}
		{/if}
	</div>
	...
	{#if footer}
		<div
			class="flex min-w-0 flex-wrap items-center justify-between gap-2 border-t border-divider bg-surface-primary px-3 py-2 text-[11px] text-text-muted select-none sm:gap-4 sm:px-4"
		>
			{@render footer()}
		</div>
	{/if}
